...
void show(WordCounter *pWordcounter);        /* Outputs a word and its count of occurrences */
...
void show(WordCounter *pWordcounter)
{
    /* output the word left-justified in a fixed field width followed by the count */
    printf("%-20s   %5d\n", pWordcounter->word,pWordcounter->word_count);
}

...
BOOL JudgeFreshHighScore(){
	// 返回FALSE则是随机成绩
	if (step!= step_after_random && step_after_random < RANDOM_STEP_THRESHOLD) {
		// 必须达到撤销存储步骤数才算玩家自己打的，否则名字为“随机”
...
void JudgeAction(HWND hwnd, BOOL move){
	...
	if (move) {
		NewNum(TRUE);
		NewRecord();
		if (redo_score >= REDO_SHRESHOLD) {
			// 每得一万分得一次撤销机会，一次合成超过了1万分也只计一次
		...
		if (onrandom == FALSE) {
			PlaySound((LPCTSTR)IDR_WAVE1, hInst, SND_RESOURCE | SND_ASYNC | SND_NOSTOP);
		}
		if (isDead()) {
			if (onrandom != TRUE) {
				// 正常死亡
				if (InHighScore() && score >= HIGHSCORE_THRESHOLD) {
					// 进入高分榜，大于1000分
					if (can_redo == 0) {
						if (JudgeFreshHighScore() == FALSE) {
							// 随机成绩，非随机成绩弹出排行榜，手动清零
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam){
	...
	case WM_CREATE:{
			...
			{
				FILE *file;
				if (_tfopen_s(&file, szFilePath, TEXT("r, ccs = UNICODE")) == 0) {
					// 成功则为0
...
void SaveGame(){
	FILE *file;
	if (_tfopen_s(&file, szFilePath, TEXT("w+, ccs = UNICODE")) == 0) {
		// 成功则为0
...
LRESULT CALLBACK ChildWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_LBUTTONDOWN:
		...
		case RANDOMGAME:
			...
			while (redo_count > (REDO_NUM - 1) - STEP_BEFORE_DEAD) {
				// 留死前N步
				Redo();
				redo_count--;
				step--;
